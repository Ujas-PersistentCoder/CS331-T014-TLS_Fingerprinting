...
struct FlowHash {
    std::size_t operator()(const FlowKey &k) const {
        ...
        if (k.ip_version == 4) {
            ...
            const uint32_t *p1 = reinterpret_cast<const uint32_t*>(&k.src_ip.v6);
            const uint32_t *p2 = reinterpret_cast<const uint32_t*>(&k.dst_ip.v6);
...
struct StreamBuffer {
    // 4096 bytes comfortably fits Post-Quantum Kyber handshakes while sparing CPU cache
